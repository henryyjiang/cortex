...
expected format â€” the old single-greedy-token exact-match scoring pinned every
model to 0% and was uninformative.

Multi-pass knobs (cross-state models only):
  --passes_per_chunk N   each priming chunk gets N full-model passes
                         (M_cross carried pass-to-pass â€” multi-pass fill)
...
from model_utils import (load_checkpoint, prime_cross_state, greedy_generate,
                         ccot_prime)
...
def split_context(tokenizer, context: str, suffix: str, seq_len: int,
    ...
    window as ceil(total/num_chunks) — capped at seq_len."""
    ...
    # the END (question + "Answer:") â€” degrade by dropping instruction/demos.
...
def eval_one(model, tokenizer, context, question, answer, T, seq_len,
    ...
    # Prime the cross state on every context chunk before the final one â€” the
...
def run_task(task_name, model, tokenizer, T, seq_len, max_examples, length_buckets,
    ...
    for cfg in config_names:
        if dataset_path is not None:
            ...
            if not local.exists():
                # A missing local file is a setup error, not a skippable bucket â€”
            ...
            try:
                ...
                print(f"  [{task_name}/{cfg}] ERROR loading from hub â€” {e}")
                continue
...
def main() -> None:
    ...
    print(f"T={T if T is not None else cfg.mean_recurrence}  tasks={args.tasks}  "
          f"passes_per_chunk={args.passes_per_chunk}  ccot_passes={args.ccot_passes}")
    ...
    if total_examples == 0:
        print("ERROR: 0 examples were evaluated across all tasks/buckets â€” "
