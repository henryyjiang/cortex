...
# ── Easy evals, single model: ccot-5b ───────────────────────────────────────
...
# it was killed while loading THIS model.
#
...
#   EVAL_TAG=$(date +%Y%m%d) sbatch job_eval_easy_ccot.sh
...
MODEL="ccot-5b"
RUN_DIR="runs/ccot-5b"      # full-network continuous-CoT (looped) variant
